...
export function assessHealth(
  ...
  for (const ch of bp.channels) {
    ...
    if (ch.pin_template && live.type === 0) {
      ...
      if (!canPin && (live.messages.length === 0 || !pinned)) {
        findings.push({
          severity: "warn",
          code: "pin.unpinnable",
          target: ch.key,
          detail: "sticky pins need PIN_MESSAGES or MANAGE_MESSAGES — not in the least-privilege integer; templates stay in-channel unpinned",
        });
      } else if (live.messages.length > 0 && !pinned) {
  ...
  if (extra?.administrator || extra?.botPermissions !== undefined) {
    ...
    if (extras.length) {
      findings.push({
        severity: "warn",
        code: "perms.excess",
        detail: `Extra bits ${extras.join(", ")}. Not required. Do not add Administrator.`,
      });
    }
  }
